...
class DockerRegistry {
  constructor(imageRef, username, password, insecure) {
    ...
    this.registry = drc.createClientV2(_extends({
  ...
  getLayers(maxSchemaVersion = 2) {
    ...
    return new Promise((resolve, reject) => this.registry.getManifest({ ref, maxSchemaVersion }, (err, manifest) => {
      if (err) {
        debug(err);
  ...
  _getRegistryBase() {
    var _repoAndRef = this.repoAndRef;
    const name = _repoAndRef.index.name,
          imageName = _repoAndRef.remoteName;


    const proto = this.insecure ? 'http' : 'https';

    const dockerHub = 'registry.hub.docker.com';
    const registryUrl = this.isPublic ? dockerHub : name;
  ...
  buildLayersV1({ fsLayers }) {
    ...
    return fsLayers.reverse().map(({ blobSum }) => ({
