...
function testRepository(req, res) {
    ...
    .then(function (doc) {
        return new Promise(function (resolve, reject) {
            info(`DB: complete`);
            if (doc) { tempDoc = doc; resolve(doc); }
            else { throw new dbException(`${req.query.repository}: No such repository`); }
        });
    })
...
function addRepository(req, res) {
    if (req.body.port <= 0) { resErr(res, new paramsException(`port illegal`)); return; }
    repository.findOne({ repository: req.body.repository })
    .then(function (doc) {
        return new Promise(function (resolve, reject) {
            if (doc) { throw new dbException(`${req.body.repository} already existed, cannot add`); }
            else { resolve(); }
        })
    ...
    .then(function () {
        return new Promise(function (resolve, reject) {
            ...
            if (!repositoryClone.port) { delete repositoryClone.port; }
            if (!repositoryClone.username) { delete repositoryClone.username; }
            if (!repositoryClone.passwd) { delete repositoryClone.passwd; }
            if (!repositoryClone.isHttps) { delete repositoryClone.isHttps; }
            debug(JSON.stringify(repositoryClone));
            repository.create(repositoryClone)
            .then(function (data) {
                info(`DB: compelete`);
                resolve(data);
            }, function (err) { throw new dbException(err); })
...
function removeRepository(req, res) {
    ...
    .then(function (doc) {
        return new Promise(function (resolve, reject) {
            ...
            else { throw new dbException(`No ${req.query.repository}, cannot remove`); }
        })
    }, function (error) { throw new dbException(error); })
    .then(function (data) {
...
function setRepository(req, res) {
    if (req.body.port <= 0) { resErr(res, new paramsException(`port illegal`)); return; }
    repository.findOneAndUpdate({ repository: req.query.repository }, { $set: req.body })
    .then(function (doc) {
        return new Promise(function (resolve, reject) {
            info(`DB: complete`);
            if (doc) {
