    ...
    return new Promise(function (resolve, reject) {
        ...
        request.get(option, function (err, response, data) {
            ...
            if (!err && response.statusCode === 200) {
                ...
                if (encoding == 'gzip') {
                    zlib.gunzip(bufer, function (err, decoded) {
                        if (err) { reject(`unzip error ${err}`); }
                        if (decoded) { resolve(decoded.toStrign()); return; }
                    });
                } else if (encoding == 'deflate') {
                    zlib.inflate(buffer, function (err, decoded) {
                        if (err) { reject(`deflate error ${err}`); }
                        if (decoded) { resolve(decoded.toString()); return; }
                    });
                } else {
