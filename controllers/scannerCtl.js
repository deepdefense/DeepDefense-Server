...
function getPage(req, res) {
    ...
    let and = req.body.search && req.body.search.length !== 0 ? req.body.search.map(function (key) {
        return { name: { $regex: key } };
    }) : null;
    let sortOption = {};
    sortOption[field] = order;
    dockerImage.find(and ? { $and: and, repository: req.body.repository } : { repository: req.body.repository })
    .sort(sortOption)
    .skip(from)
    .limit(size)
    .select({
    ...
    .exec(async function (err, docs) {
        ...
        resSuc(res, { docs, count: await dockerImage.find(and ? { $and: and, repository: req.body.repository } : { repository: req.body.repository }).count() });
    });
}
...
function getImage(req, res) {
    ...
    .then(function (doc) {
        ...
        else { throw new dbException(`No such image`); }
    })
    .catch(function (err) {
        warn(`getImage: faild`);
        resErr(res, doc);
...
function getImagePage(req, res) {
    ...
    .exec(async function (err, docs) {
        ...
        resSuc(res, {
            docs,
            count: await dockerVulnerability.find({
                repository: req.body.repository,
                image: req.body.image,
                tag: req.body.tag,
            }).find(and ? { $and: and } : {}).count()
        });
    });
}
